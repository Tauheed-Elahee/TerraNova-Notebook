...
**Initialisation and the distribution mismatch.** New token embeddings cannot simply be initialised from Stage 1 layer-12 vectors: layer-12 residual stream representations are in a different distribution from layer-0 input embeddings, so feeding a layer-12 vector into layer-0 places the model out of distribution for the entire forward pass. The correct approach is Token Distillation (2505.20133), which trains new input embeddings to reproduce the mid-layer attention patterns of the original multi-token sequence. This is not merely complementary to Stage 2 — it is the required method for Stage 2 to be a valid experiment.
